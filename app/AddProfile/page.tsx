...
//     // Testing to create profile when signing up with Clerk
//     // This useEffect is to handle click outside to close the modal, unrelated to profile submission
//     useEffect(() => {
//         if (isSignedIn && isLoaded && user) {
//             // Assuming 'user' contains firstName, lastName, and primary email address.
//             // Adjust according to the actual data structure provided by Clerk.
...
//                 name: fullName,
//                 email: primaryEmail,
//                 // You can set other fields based on available user information
//             }));
//         }
...
//     //Testing orginal at bottom commented out
//     const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
//         e.preventDefault();

//         // Fetch existing profiles from local storage
//         const profiles = JSON.parse(localStorage.getItem('profiles') || '[]');

//         // Check if there is already a profile with the same name and email
//         const profileExists = profiles.some((profile: Profile) => profile.email === newProfile.email && profile.name === newProfile.name);

//         if (profileExists) {
//             // If a matching profile is found, show an error message
//             setSubmissionMessageType('error');
//             setSubmissionMessage('A profile with the same name and email already exists!');
//             setTimeout(() => setSubmissionMessage(''), 5000);
//             return;
//         }

//         // If no matching profile is found, add the new profile and show a success message
//         const profileToSave = { ...newProfile, backgroundImageUrl };
//         localStorage.setItem('profiles', JSON.stringify([...profiles, profileToSave]));
...
//         setSubmissionMessageType('success');
//         setSubmissionMessage('Profile successfully added!');
//         setTimeout(() => setSubmissionMessage(''), 5000);
//     };
...
//                 setSelectedProfileImageName(file.name); // Update state with file name
//             } else { // Assuming you want to handle 'background' type
//                 setBackgroundImageUrl(imageUrl);
//                 setSelectedBackgroundImageName(file.name); // Update state with file name
//             }
//         }
//     };

//     // Update online change
//     const handleOnlineChange = (e: React.ChangeEvent<HTMLInputElement>) => {
//         setNewProfile({ ...newProfile, isOnline: e.target.checked });
//     };
...
//                 {/* is online check mark test */}
//                 <div className="form-check">
//                     <input type="checkbox" name="isOnline" checked={newProfile.isOnline} onChange={handleOnlineChange} className="form-checkbox h-5 w-5 text-blue-600" /><span className="ml-2 text-gray-100">Test: Is Online?</span>
//                 </div>
...
//                                 maxLength={500} // Limit introduction text length
//                             ></textarea>
...
//                                 <label className="mr-1 mb-1 ease-linear transition-all duration-150 rounded shadow uppercase text-sm px-6 py-3 font-bold  block text-center bg-green-500 text-white cursor-pointer hover:bg-green-600 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-opacity-50">
//                                     Upload Background Image
//                                     <input type="file" onChange={(e) => handleImageChange(e, 'background')} className="hidden" accept="image/*" />
//                                 </label>
//                                 {selectedBackgroundImageName && (
//                                     <div className="flex items-center justify-center py-2 px-4 bg-green-100 text-green-800 rounded-lg mt-2">
//                                         {/* <svg className="w-4 h-4 text-green-500 mr-2" fill="none" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewBox="0 0 24 24" stroke="currentColor">
